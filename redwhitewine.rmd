...
on all of the investigated variables, with the possible exeptions of _alcohol_
and _quality_.
Curiously, both of these exceptions appear to be skewed in favor of larger
values for the white variaty.
...
Overall, only alcohol seems to have any kind of significant relationship with
_quality_.
This looks to be the case for white wines, which are overrepresented in the
data set.
For red wines, _alcohol_, _sulphates_ and _citric.acid_ seemed to relate to
_quality_.
...
Strangely, no. Even _sulphates_ and _citric.acid_ that are both related to 
_quality_ don't seem to relate to one another.
...
```{r echo=FALSE, Citric_Sulphates}
ggplot(wine, aes(citric.acid, sulphates)) +
  geom_point(aes(color = quality.as.factor)) +
  facet_wrap( ~ variant) +
  scale_x_continuous(limits = c(0,1))

ggplot(subset(wine, quality < 5 | quality > 6), aes(citric.acid, sulphates)) +
  geom_point(aes(color = quality.as.factor)) +
  facet_wrap( ~ variant)
```
...
```{r echo=FALSE}
ggplot(wine, aes(citric.acid, alcohol)) +
  geom_point(aes(color = quality.as.factor)) 
...
```{r echo=FALSE}
ggplot(wine, aes(sulphates, alcohol)) +
  geom_point(aes(color = quality.as.factor)) 
