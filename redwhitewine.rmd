...
---
Fabio Tosetto Reale - Exploration on _Vinho Verde_ Wine
========================================================
...
Interesting, I expected _residual.sugar_ to inversely correlate strongly with _alcohol_ since the sugar is converted to alcohol in the fermentation process.
But that doesn't seem likely anymore, due to how different the histograms look.

Also, dry wines are likely the ones with the least amounts of _residual.sugar_, so it looks like white wines are more likely sweet then red wines.
...
It seemsa _total.sulfur.dioxide_ is bimodal. This might be a distinguishing feature for red vs white wines.
...
# After some consideration, I decided not to include this next plot
# but I'll leave it here as a reference
...
All of these exceptions appear to be skewed in favor of larger values for the white variaty. This goes against my intuition in the cases of _alcohol_ and
_quality_
...
help.
Unfortunately, none of the variables are related to _tannin_ as far as I could find. These are supposed to be complex organic molecules and don't seem to relate to any of the chemical quantities described in the data.
...
In all of the above boxplots there is an omission, a white wine outlier with 65.8 g/L in _residual sugar_.
...
In all of the density plots there is an omission, the same white wine outlier with 65.8 g/L in _residual sugar_, which has a _density_ of 1.03898 g/L.
...
density.
One of these behaviours seem to follow closely what happens with reds, but 
in lower densities.
...
red wines, and these are between _quality_ and _alcohol_, _sulphates_ and _citric.acid_.

As for distinguishing the _variant_ variable, in adition to _sulphates_ and _citric.acid_, _residual.sugar_ are likely distributed differently between
variants.

### Did you observe any interesting relationships between the other features \
(not the main feature(s) of interest)?

Yes. The variables _density_ and _residual.sugar_ seem to have distinct distributions for red and white wines. More over, there seems to be 2 distinct relationships between these variables within white wines.
...
Let's say wines with less then 3 g/l in _residual.sugar_ are *dry* wines and the rest are *sweet* wines. Let's check that we can distinguish red from white *dry* wines.

```{r echo=FALSE, Multivariate_Sugar_Density2}
ggplot(subset(wine, residual.sugar < 3),
       aes(residual.sugar, density)) +
  geom_jitter(height = 0, aes(color = variant)) +
  scale_color_manual(values=c(redw.col, whitew.col))
...
When restricting to *sweet* wines, this low _density_ higher _residual.sugar_ relationship looks stronger.

```{r echo=FALSE}
# I just eye-balled angular coeficients for somewhat constant
# linear relationships for density and residual.sugar
wine <- transform(wine, dry.density = 1000*density - 4*residual.sugar)
wine <- transform(wine, sweet.density = 1000*density - 0.533*residual.sugar)

ggplot(subset(wine, residual.sugar < 3),
              aes(alcohol, dry.density)) +
  geom_point(aes(color = quality.as.factor)) +
  facet_wrap( ~ variant)

ggplot(subset(wine, residual.sugar>3 & residual.sugar<25 & variant=="white"),
              aes(alcohol, sweet.density)) +
  geom_point(aes(color = quality.as.factor))
```
...
Yes. There are 2 distinct behaviours in the relation between _density_ and 
_residual.sugar_. I believe these distinct behaviours distinguish dry and sweet
wines.
...
pl1 <- ggplot(subset(wine, residual.sugar < 25),
              aes(residual.sugar, density)) +
...
pl2 <- ggplot(subset(wine, residual.sugar < 25),
              aes(residual.sugar, density)) +
...
# Not sure if this fits somewhere
